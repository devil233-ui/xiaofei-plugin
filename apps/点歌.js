...
async function music_search(e, search, source, page = 1, page_size = 10) {
    ...
    if (result && result.data && result.data.length > 0) {
        ...
        if (list.length > 1 && search) {
            // 提取纯文本（剔除所有标点、空格、特殊符号），实现无视标点精准匹配
            let cleanStr = (s) => String(s).replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g, "").toLowerCase();
            let keyword = cleanStr(search);
            const priority_artists = [ "HOYO-MiX", "米哈游", "Mihoyo", "三Z-STUDIO", "陈致逸" ];

            list.sort((a, b) => {
                let nameA = cleanStr(a.name);
                let nameB = cleanStr(b.name);
                let artistA = String(a.artist);
                let artistB = String(b.artist);
                
                let hasA = keyword.length > 0 ? nameA.includes(keyword) : false;
                let hasB = keyword.length > 0 ? nameB.includes(keyword) : false;
